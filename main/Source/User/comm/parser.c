...
static const SParserRxList_T parserRxList[] = 
{
    ...
    { TIMER_USER_ID_COMM_COMP_RX,   COMM_ID_COMP,  IsValidPktComp,   ParserPktComp  },
    { TIMER_USER_ID_COMM_EOL_RX,    COMM_ID_EOL,   IsValidPktEol,    ParserPktEol   }
};
...
{
    { TIMER_USER_ID_COMM_FRONT_TX,  COMM_ID_FRONT,    MakePktFront,    Crc16Front  },
    { TIMER_USER_ID_COMM_COMP_TX,   COMM_ID_COMP,     MakePktComp,     Crc16Comp   },
    { TIMER_USER_ID_COMM_EOL_TX,    COMM_ID_EOL,      MakePktEol,      Crc16Eol    }
...
void SendPacketHandler( void )
{
    ...
    for( i=0 ; i<SZ_PARSER_TX; i++ )
    {
        ...
        if( IsExpiredTimer(TIMER_USER, list->timerId) == TIMER_EXPIRE )
        {
            ...
            pComm = GetCommHeader(list->commId);

            /* MAKE PACKET */
            MEMSET( (void __FAR*)pktSend, 0, PARSER_TX_BUF_SZ );
            pktSendLen = list->makePkt((CommHeader_T *)pComm, pktSend);
            if( pktSendLen > 0 )
            {
                /* MAKE CRC-16 */
